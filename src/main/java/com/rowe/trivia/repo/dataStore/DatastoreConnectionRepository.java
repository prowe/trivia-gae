...
class DatastoreConnectionRepository implements ConnectionRepository {        
        private static final Logger log = LoggerFactory.getLogger(DatastoreConnectionRepository.class);
        private static final String KIND = "UserConnection";
        ...
        public DatastoreConnectionRepository(String userId, ConnectionFactoryLocator connectionFactoryLocator, TextEncryptor textEncryptor, DatastoreService datastore)
        {
            this.userId = userId;
            this.connectionFactoryLocator = connectionFactoryLocator;
            this.textEncryptor = textEncryptor;
            this.datastore = datastore;
            this.userKey = KeyFactory.createKey(User.class.getSimpleName(), userId);
        }
        ...
        public MultiValueMap<String, Connection<?>> findAllConnections() {
                ...
                for (Entity entity : datastore.prepare(query).asIterable()) {
                    Connection<?> connection = entityToConnection(entity);
        ...
        public MultiValueMap<String, Connection<?>> findConnectionsToUsers(MultiValueMap<String, String> providerUserIds) {
                ...
                for (Entity entity : queryResultSet) {
                       resultList.add(entityToConnection(entity));
                }
        ...
        public Connection<?> getConnection(ConnectionKey connectionKey) {
                ...
                try {
                        ...
                        log.warn("Too many results were found for query " + query.toString());
                        throw new NoSuchConnectionException(connectionKey);
                }
                if (singleEntity == null) throw new NoSuchConnectionException(connectionKey);
                return entityToConnection(singleEntity);
        }
        ...
        private Connection<?> findPrimaryConnection(String providerId) {
                ...
                if(it.hasNext()){
                	return entityToConnection(it.next());
                }else{
                	return null;
                }
        }
        ...
        public void addConnection(Connection<?> connection) {
                ...
                String connectionKeyName = createConnectionKeyName(connection.getKey());
                Entity userConnection = new Entity(KIND, connectionKeyName, userKey);
...
//                for (ConnectInterceptor interceptor : interceptingConnectionsTo(connection)) {
//                        interceptor.beforeCreate(userId, connection);
//                }
                
                final Key key = new KeyFactory.Builder(userKey).addChild(KIND, connectionKeyName).getKey();
...
//                }
                String connectionKeyName = createConnectionKeyName(connection.getKey());
                final Key connectionKey = new KeyFactory.Builder(userKey).addChild(KIND, connectionKeyName).getKey();
                Transaction txn = datastore.beginTransaction();
                ...
                try {                        
                    ...
                    log.warn("There was the problem updating connection " + connection.getKey().toString() + ". No such connection exists.");
                } finally {
                    if (txn.isActive()) {
		                txn.rollback();
		                updated = false;
...
//        }
        ...
        public void removeConnections(String providerId) {
                ...
                /*for (ConnectInterceptor interceptor : interceptingConnectionsTo(connections.iterator().next())) {
...
//                }
        ...
        public void removeConnection(ConnectionKey connectionKey) {
                String connectionKeyName = createConnectionKeyName(connectionKey);
                final Key key = new KeyFactory.Builder(userKey).addChild(KIND, connectionKeyName).getKey();
                Transaction txn = datastore.beginTransaction();
                try {                        
...
//                        }
                        ...
                        log.warn("There was problem deleted connection " + connectionKey.toString() + ". No such connection exists.");                        
                } finally {
                        if (txn.isActive()) txn.rollback();
                }                
        }
        
        private Connection<?> entityToConnection(Entity entity){
        ...
        private Entity connectionToEntity(Connection<?> connection){
        	//TODO: populate
        	return null;
        }
        ...
        public static String createConnectionKeyName(ConnectionKey connectionKey) {
        ...
        /**
         * Adds a ConnectInterceptor to receive callbacks during the connection CRUD operations.
        ...
        private List<ConnectInterceptor<?>> interceptingConnectionsTo(Connection<?> connection) {
